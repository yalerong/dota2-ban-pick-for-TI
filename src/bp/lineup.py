...
def build_tables(matches: list[LineupMatch], k: float, k_single: float = 20) -> Tables:
...
def oof_features(matches: list[LineupMatch], k: float, folds: int, roles, targets, k_single: float) -> list[dict]:
    ...
    for hold in _folds(len(matches), folds):
        hs = set(hold.tolist())
        t = build_tables([m for i, m in enumerate(matches) if i not in hs], k, k_single)
        for i in hold:
...
def evaluate(model: LineupModel, tests: list[LineupMatch], bins: int = 8) -> dict:
    ...
    return {"n": len(tests), "base_rate": base, "model": summary(p), "constant": summary(p_const), "hero_only": summary(p_hero),
            ...
            "k_search": model.k_search, "n_train": model.n_train,
